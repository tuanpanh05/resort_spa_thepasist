...
> [!NOTE]
> **Ánh xạ trực tiếp:** `[Mã yêu cầu]` $\rightarrow$ `[Thành phần Code]` $\rightarrow$ `[Mục tiêu Tuân thủ]`.
...
#### Bối cảnh (Context)
*Mô tả vấn đề hoặc áp lực buộc phải đưa ra quyết định này. Bối cảnh kỹ thuật hoặc pháp lý nào dẫn đến ADR này?*
...
* Dự kiến tải trong 12 tháng tới: `[X]` users, `[Y]` consent records/day.
* Giải pháp scale: `[horizontal / vertical / caching strategy]`.
...
*(Vẽ sơ đồ luồng xử lý lỗi hoặc exception flow tương tự Happy Path khi có rủi ro nghiệp vụ)*

---

## 6.3. State Machine
*(Bắt buộc nếu module có trạng thái)*

```plantuml
@startuml
[*] --> PENDING : [Trigger khởi tạo]
PENDING --> ACTIVE : [Điều kiện chuyển tiếp / Action thực hiện]
ACTIVE --> REVOKED : [Điều kiện] / [Action]
ACTIVE --> EXPIRED : [Điều kiện] - vd: TTL reached / [Action]
...
export interface I[ModuleName]Repository {
  ...
  save(entity: [Entity]): Promise<[Entity]>;
  // Lưu ý: Không có delete() - Append-only nếu là module PII
}
```
...
*(bắt buộc tick trước khi chạy migration)*
...
#### [POST] Thiếu required field $\rightarrow$ 400
```bash
curl -X POST https://[host]/api/v1/[resource] \
  -H "Authorization: Bearer [JWT_TOKEN]" \
  -H "Content-Type: application/json" \
...
#### [GET] Không có JWT $\rightarrow$ 401
```bash
...
* **Chú thích:**
  ...
  * **All** = Được phép trên toàn bộ dữ liệu
...
| **Append-only** | Chiến lược lưu trữ không cho phép UPDATE/DELETE, chỉ cho phép INSERT |
