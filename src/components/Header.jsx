...
export default function Header() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${showGlass ? "glass-header shadow-sm py-4" : "bg-gradient-to-b from-black/60 to-transparent py-6"}`}
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        <div className="flex items-center justify-between h-12">
          ...
          >
            <div
              className={`relative p-2 rounded-full transition-colors duration-300 ${showGlass ? "bg-primary-100 text-primary-800" : "bg-white/10 text-white"}`}
            >
          ...
          {/* Center Side: Simplified Menu Items */}
          <nav className="hidden xl:flex space-x-8 font-medium text-sm tracking-wide flex-shrink-0">
            {navItems.map((item, index) => (
              <Link
                key={index}
                to={item.href}
                className={`whitespace-nowrap relative py-1 transition-colors duration-300 ${showGlass ? "text-sage-700 hover:text-primary-900" : "text-white/80 hover:text-white"}`}
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="hidden xl:flex items-center space-x-2 flex-shrink-0">
            ...
            <Link
              to="/dang-ky"
              className={`whitespace-nowrap px-5 py-2.5 rounded-none text-xs font-semibold tracking-wider transition-all duration-300 hover:scale-105 ${showGlass ? "bg-primary-800 hover:bg-primary-900 text-white" : "bg-white hover:bg-white/95 text-primary-950"}`}
            >
              Đăng ký
            </Link>
          </div>
          ...
          <div className="flex xl:hidden">
            <button
              ...
              type="button"
              className={`inline-flex items-center justify-center p-2 rounded-md transition-colors ${showGlass ? "text-sage-800 hover:bg-primary-50" : "text-white hover:bg-white/10"}`}
        ...
        <div className="px-6 py-6 space-y-1 bg-white shadow-lg">
          {navItems.map((item, index) => (
            <Link
              ...
              className="block px-3 py-2.5 rounded-md text-sm font-medium text-sage-800 hover:bg-primary-50 hover:text-primary-900"
            >
              {item.label}
            </Link>
          ))}
          <div className="pt-4 border-t border-primary-50 flex flex-col space-y-2">
            <Link
              to="/dang-nhap"
              onClick={() => setIsOpen(false)}
              className="w-full text-center py-2.5 text-sm font-medium text-sage-700 hover:text-primary-900"
            >
              Đăng nhập
            </Link>
            <Link
              to="/dang-ky"
              onClick={() => setIsOpen(false)}
              className="w-full text-center py-2.5 rounded-none text-sm font-medium bg-primary-800 hover:bg-primary-900 text-white shadow-sm"
            >
              Đăng ký
            </Link>
          </div>
