...
:populate_windows
sqlcmd -S "%DB_HOST%" -E -i "%~dp003-Design\database\ResortSpaDB_Master.sql"
if %errorlevel% equ 0 goto populate_done
sqlcmd -S "%DB_HOST%,%DB_PORT%" -E -i "%~dp003-Design\database\ResortSpaDB_Master.sql"
goto populate_done

:populate_sql
sqlcmd -S "%DB_HOST%" -U "%DB_USER%" -P "%DB_PASS%" -i "%~dp003-Design\database\ResortSpaDB_Master.sql"
if %errorlevel% equ 0 goto populate_done
sqlcmd -S "%DB_HOST%,%DB_PORT%" -U "%DB_USER%" -P "%DB_PASS%" -i "%~dp003-Design\database\ResortSpaDB_Master.sql"
goto populate_done
