...
> **Thứ tự bắt buộc:** viết test (`.spec.ts` / `.spec.tsx`) $\rightarrow$ chạy $\rightarrow$ xác nhận **FAIL** 🔴 $\rightarrow$ implement $\rightarrow$ **PASS** 🟢 $\rightarrow$ **refactor** 🔵.
...
| **Priority** | 🔴 P0 / 🟠 P1 / 🟡 P2 |
| **Sprint** | S[N] (YYYY-MM-DD $\rightarrow$ YYYY-MM-DD) |
...
* **Severity:** HIGH
* **Feature Under Test:** Full flow: [bước đầu $\rightarrow$ bước cuối]
...
> [!NOTE]
> Gap vẫn **OPEN** $\rightarrow$ giữ nguyên entry trong `PHASE_GAP_ANALYSIS.md`.
